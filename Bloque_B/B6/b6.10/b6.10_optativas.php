...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    echo $asignatura . in_array($asignatura, $optativas);
    if (in_array($asignatura, $optativas)) {

        $sms = 'Gracias por la seleccion';
        $valido = true;
        // Si queremos Redirigir a una página de confirmación
...
<form action="b6.10_optativas.php" method="POST">
    ...
    <?php foreach ($optativas as $opcion) { ?>
        <?= $opcion ?>
        <input type="radio" name="asignatura" value="<?= htmlspecialchars($opcion)?>"<?= ($asignatura == $opcion) ? 'checked' : '' ?>>
    <?php } ?>
